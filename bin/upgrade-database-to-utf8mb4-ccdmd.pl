...
sub checkAndUpdateTableColumnTypes {
	...
	for my $field (keys %$schema_field_data) {
		...
		if ($data_type ne $schema_data_type) {
			...
			eval {
				# la conversion sera fait seulement à la deuxième exécution du script
				$dbh->do("UPDATE `$table` SET `$field_name`=convert(cast(convert(`$field_name` using latin1) as binary) using utf8);");
