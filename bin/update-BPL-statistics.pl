...
$dbh->do(<<EOS);
CREATE TABLE IF NOT EXISTS `BPL_problem_user` (
...
$dbh->do(<<EOS);
DROP TABLE IF EXISTS `BPL_local_statistics`
EOS

$dbh->do(<<EOS);
CREATE TABLE `BPL_local_statistics` (
...
foreach my $courseID (@courses) {
    ...
    $dbh->do(<<EOS);
INSERT IGNORE INTO BPL_problem_user 
  (course_id,
  user_id,
  set_id,
  due_date,
...
SELECT
  ...
  REPLACE(`${courseID}_problem`.source_file,'local/BPL/','BPL/'),
...
WHERE (`${courseID}_problem`.source_file LIKE 'BPL/%'
  OR `${courseID}_problem`.source_file LIKE 'local/BPL/%')
...
# compile desired statistics from bpl problem user table.  
$dbh->do(<<EOS);
INSERT INTO BPL_local_statistics
  (source_file,
  students_attempted,
  average_attempts,
  average_status)
...
print( "You may want to run load-BPL-global-statistics.pl to update the global statistics data.\n",
	"If this is being run by BPL-update or BPLen-update that will be done automatically.\n");
