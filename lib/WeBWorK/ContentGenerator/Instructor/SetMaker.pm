...
sub make_top_row {
	...
	my $library_selected = $self->{current_library_set};
	my $set_selected = $r->param('local_sets');
	my (@dis1, @dis2, @dis3, @dis4,@dis5,@dis6,@dis7) = ();
	@dis5 =	 (-disabled=>1) if($browse_which eq 'browse_bpl_library');	 
	@dis7 =	 (-disabled=>1) if($browse_which eq 'browse_bplen_library');	 
...
sub make_data_row {
	...
	my $problem_output = $pg->{flags}->{error_flag} ?
		CGI::div({class=>"ResultsWithError"}, CGI::em($r->maketext("This problem produced an error")))
...
sub pre_header_initialize {
	...
	if ($browse_lib ne '') {
		...
		@pg_files=();
                ...
                my @dbsearch;
		@dbsearch = WeBWorK::Utils::ListingDB::getBPLENDBListings($r,0, $typ);
...
#print STDERR "COMEHERE1: $set_to_display\n";
...
sub body {
	...
	if ($last_index >= $first_index) {
		my @plist = map {$_->{filepath}} @pg_files[$first_index..$last_index];
                my $disp_mode = $r->param('mydisplayMode');
...
sub output_CSS {
  ...
  print qq{
           <link href="$webwork_htdocs_url/js/vendor/bootstrap/css/bootstrap-tagsinput.css" rel="stylesheet" type="text/css" />};
  print qq{
           <link href="$webwork_htdocs_url/js/vendor/bootstrap/css/bootstrap-tagsinput-typeahead.css" rel="stylesheet" type="text/css" />};
