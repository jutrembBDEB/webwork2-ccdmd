	...
	unless ($file_type eq 'course_info' || $file_type eq 'options_info') {
		$output_string .= join("",
			...
			CGI::div({ class => "pg_editor_new_window_div" },
				WeBWorK::CGI_labeled_input(-type => "checkbox", -id => "newViewWindow", -label_text => $r->maketext("Open in new window")))
		);
	}
...
sub save_form {
	...
	if ($self->{editFilePath} =~ /$BLANKPROBLEM$/ ) {
		...
		return $r->maketext("Save to [_1] and View", CGI::b($self->shortPath($self->{editFilePath}))) .
			CGI::div({ class => "pg_editor_new_window_div" },
				WeBWorK::CGI_labeled_input(-type => "checkbox", -id => "newSaveWindow", -label_text => $r->maketext("Open in new window")));
