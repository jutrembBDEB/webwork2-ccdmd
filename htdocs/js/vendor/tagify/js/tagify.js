...
Tagify.prototype = {
            ...
            if( value.indexOf(',') != -1 || isDatalistInput ){
                ...
                if( !showSuggestions && datalistInDOM ) {
                   this.DOM.input.parentNode.removeChild(this.DOM.datalist);
                    ...
                    this.DOM.datalist.innerHTML = "<label> \
                    ...
                    for( i=this.settings.whitelist.length; i--; ) {
                          ...
                          var x = patt.test(this.settings.whitelist[i]);
                          if(x) 
                              OPTIONS += "<option>"+ this.settings.whitelist[i] +"</option>";
                    }

                    this.DOM.datalist.innerHTML = this.DOM.datalist.innerHTML.replace('[OPTIONS]', OPTIONS);
    ...
    clearTags : function(){
        ...
        var i;
        var taglength = this.DOM.scope.querySelectorAll('tag:not(.tagify--hide)').length;
        alert(taglength);
        for( i=taglength; i--; ) {
            this.removeTag(i,false);
        }
